...
| ST-CONTROL | Opt-in control channel for input injection, frame stepping, counters, capture | partial | Built on `lucent::http::Server`, loopback only, opened only when `WIIUPORT_CONTROL_PORT` is set. `GET /counters` is live and answered 12 of 12 polls against a running title. Exact gap: counters are the only route. No input injection, no frame stepping, no screenshot or capture endpoint, so a tool can watch the product but cannot yet drive it -- which is why no run has reached in-game control. |
