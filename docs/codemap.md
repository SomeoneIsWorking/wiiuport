...
| Blending the vertices the title rewrote | `src/wiiuport/interp/VertexBlend.h` | Keeps each of the title's draws' vertex bytes by the vertex-stage object it drew; at replay, lerps float attributes against the draw of that object's planner partner in N-1 and hands them to the fork's `VertexReplacements`, which binds them from a private ring (host-memory vertex path only). Counted by outcome in `GET /interpolation`. |
