...
| Blending the vertices the title rewrote | `src/wiiuport/interp/VertexBlend.h` | Keeps each of the title's draws' vertex bytes by the vertex-stage object it drew; at replay, lerps the float attributes that moved since N-2 against the draw of that object's planner partner in N-1 (where the object is told apart only by its place, the N-2 and N-1 siblings its vertices most resemble, the N-1 one checked by `Midpoint`) and hands them to the fork's `VertexReplacements`, which binds them from a private ring (host-memory vertex path only). Counted by outcome in `GET /interpolation`, and by vertex shader in `GET /vertices` (read by `tools/wiiuport/vertex_census.py`). |
