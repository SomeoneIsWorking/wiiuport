...
| Holding the title between frames | `src/wiiuport/frame/FrameGate.h` | Pauses at a frame's end, steps N frames and resumes, for `POST /gate`; the last frame-shown listener, released by `ShellHost::shutdown`. Covered by `tests/cxx/gate_tests.cpp` on a thread standing in for the renderer. |
