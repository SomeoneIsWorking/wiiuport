...
| Which of the title's own objects a draw drew | `src/wiiuport/interp/DrawObjects.h`; title side `src/wiiuport/guest/ParticleProbe.h`, `WaveProbe.h` (read guest memory through the fork), `Particles.h` (pure) | `DrawObjects` names the title's object (guest address, age) behind a draw's vertex buffer; `VertexBlend` partners such a draw with the same object's younger draws. `ParticleProbe` watches Wind Waker HD's particle commit (`0x02825158`, called by the ripple draw and every JPA draw executor) and `WaveProbe` its vertex-buffer flush (`0x027b5e94`) from the sea-wave writer's call site, and both record into `Particles` each object's buffer and the quad it wrote there, which verifies a draw by its first 80 bytes being that quad. Counted in `GET /interpolation` (`particles`, `guestObjectDraws`); covered by `tests/cxx/particles_tests.cpp`. |
