...
| Fork interposition hooks | `external/cemu`, minimal and per-cause | The fork exposes callbacks at the frame boundary and at `LatteCP_itIndirectBuffer`, and `GuestCallProbes` (`Cafe/HW/Espresso`), which branches a registered function of the title's to a stub that reports each call's registers, runs the instruction it displaced and resumes the function, installed once the title is linked; it does not implement recording, blending, or policy. |
...
| Which of the title's own objects a draw drew | `src/wiiuport/interp/DrawObjects.h`; title side `src/wiiuport/guest/ParticleProbe.h` (reads guest memory through the fork), `Particles.h` (pure) | `DrawObjects` names the title's object (guest address, age) behind a draw's vertex buffer; `VertexBlend` partners such a draw with the same object's younger draws. `ParticleProbe` watches Wind Waker HD's particle commit (`0x02825158`, called by the ripple draw and every JPA draw executor) and records into `Particles` each particle's buffer and the quad it wrote there, which verifies a draw by its first 80 bytes being that quad. Counted in `GET /interpolation` (`particles`, `guestObjectDraws`); covered by `tests/cxx/particles_tests.cpp`. |
