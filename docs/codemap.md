...
| Which draws no uniform blend can move | `src/wiiuport/frame/UniformlessDraws.h`, `tools/wiiuport/draws.py` | The title's draws whose vertex shader reads no uniforms, by vertex shader, from the fork's `OnDrawPrepared`, and how many read vertex bytes that changed from the frame before; owned by `RecordingObserver`, read with `GET /draws`, diffed over a stretch of play. |
