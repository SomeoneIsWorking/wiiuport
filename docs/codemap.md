...
| Fork interposition hooks | `external/cemu`, minimal and per-cause | The fork exposes callbacks at the frame boundary and at `LatteCP_itIndirectBuffer`, and `GuestCallProbes` (`Cafe/HW/Espresso`), which routes a registered vtable slot of the title through a stub that reports each call's registers before the original runs, installed once the title is linked; it does not implement recording, blending, or policy. |
...
| Which of the title's own objects a draw drew | `src/wiiuport/interp/DrawObjects.h`; title side `src/wiiuport/guest/RippleProbe.h` (reads guest memory through the fork), `RippleParticles.h` (pure) | `DrawObjects` names the title's object (guest address, age) behind a draw's vertex buffer; `VertexBlend` partners such a draw with the same object's younger draws. `RippleProbe` watches Wind Waker HD's `dPa_ripplePcallBack::draw` and records into `RippleParticles` each particle's buffer, which verifies a draw by its quad being centred on the particle. Counted in `GET /interpolation` (`rippleParticles`, `guestObjectDraws`); covered by `tests/cxx/ripple_particles_tests.cpp`. |
