...
| Interpolating every frame | `src/wiiuport/interp/ContinuousInterpolator.h` | At each finished guest frame: blended replay, present, restore the guest's frame. Counts every tick it did not interpolate by reason and times each phase. Owns no pacing: the display's FIFO present mode spaces the frames. |
...
| One frame's draws, indexed by identity | `src/wiiuport/interp/KeyedFrame.h` | Built draw by draw, then looked up by key, by shader, or by sourced block address. |
...
| Keeping consecutive frames for offline study | `src/wiiuport/frame/RecordingSnapshot.h` | Armed over the channel; copies the next K frames' assemblies and frames them as `WIIUREC1`. `tools/wiiuport/interpolation.py` parses them. |
...
| Measuring continuous interpolation on the real title | `tools/continuous_run.py` | Plays offscreen on the GPU, reads `GET /interpolation` around a walk, keeps a snapshot, and fails when nothing was interpolated or the run rendered in software. |
