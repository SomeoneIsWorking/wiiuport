...
| Whether a draw a frame before is the one an object passed through | `src/wiiuport/interp/Midpoint.h` | Judges, value by value, whether the midpoint of what an object moved in from N-2 to N lands on a candidate in N-1; fed uniforms by `ObjectPlanner` and vertices by `VertexBlend`. Pure. |
| Values an unverified object shares with blended draws | `src/wiiuport/interp/SharedValues.h` | Indexes, per frame, the values blended draws of a shader moved in, by (position, N-2 bits, N bits), so `ObjectPlanner` draws an unverified object in what it held alike -- a pass's view -- as the blended draws are. Pure. |
...
| Blending the vertices the title rewrote | `src/wiiuport/interp/VertexBlend.h` | Keeps each of the title's draws' vertex bytes by the vertex-stage object it drew; at replay, lerps the float attributes that moved since N-2 against the draw of that object's planner partner in N-1 (where the object is told apart only by its place, the N-2 and N-1 draws of its shader its vertices most resemble, the N-1 one checked by `Midpoint`) and hands them to the fork's `VertexReplacements`, which binds them from a private ring (host-memory vertex path only). Counted by outcome in `GET /interpolation`, and by vertex shader in `GET /vertices` (read by `tools/wiiuport/vertex_census.py`). |
