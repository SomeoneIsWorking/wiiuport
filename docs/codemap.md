...
| Which values the light's map pass moved | `src/wiiuport/interp/MapPassValues.h` | Per frame, the (N-2, N) value pairs that moved in the map draws of two or more objects -- the light's rotation and cascade -- so `ObjectPlanner` holds a caster that moved only those and blends the rest with them at N, matching the look-up that is drawn at N; `ObjectPlanner::holdPixelPass` holds the same kind of value -- one two objects' pixel stages moved alike, the look-up itself -- in blended pixel stages. `POST /blends?maps=0` draws casters at N, and `pixels=0` every pixel stage as the title drew it. Covered by `tests/cxx/map_pass_tests.cpp`. |
| The light's map look-up seen through the in-between camera | `src/wiiuport/interp/LightLookUp.h` | Takes the light's axes from the frame's draws into the map and, for each row a depth-comparing pixel stage drawn at N holds, tells whether it is the light seen through the camera (in a plane of two axes and moved since N-2) and rebases it from the view at N to the in-between view; `ObjectPlanner` names the stages and their N-2 values, `ObjectBlend::rebaseLightLookUps` writes the rows. |
...
| Which of the title's own objects a draw drew | `src/wiiuport/interp/DrawObjects.h`; title side `src/wiiuport/guest/ParticleProbe.h`, `EnvironmentProbe.h`, `LineProbe.h` (read guest memory through the fork), `BufferWriters.h` (pure) | `DrawObjects` names the title's object (guest address, and an age where the title renews it in place) behind a draw's vertex buffer; `VertexBlend` partners such a draw with the same object's draws that continue as it (`GuestObject::continuesAs`). `ParticleProbe` watches Wind Waker HD's particle commit (`0x02825158`, called by the ripple draw and every JPA draw executor) and `EnvironmentProbe` its vertex-buffer flush (`0x027b5e94`) from the sea-wave and sky-cloud writers' call sites (one probe, since the fork takes one per entry), and `LineProbe` the double-buffer helper (`0x027ff1d8`) from the 3D lines' updates; all record into `BufferWriters` each object's buffer and the leading bytes it wrote there, which verifies a draw by its first 80 bytes being those. Counted in `GET /interpolation` (`bufferWriters`, `guestObjectDraws`); covered by `tests/cxx/buffer_writers_tests.cpp`. |
| Which meshes read the same vertex bytes | `src/wiiuport/interp/SharedVertexReads.h` | Groups a frame's meshes by overlapping guest ranges, and marks groups a draw the replay cannot redirect also reads, so `VertexBlend` draws a group at N whole when any of its readers is drawn at N although it moved (`sharesBytes`). Pure; covered by `tests/cxx/shared_vertex_reads_tests.cpp`. |
...
| Planning a kept snapshot again, offline | `src/wiiuport/interp/PlanReplay.h`, `tools/cxx/plan_replay.cpp`, `tools/plan_replay.py`, `tools/wiiuport/planreplay.py` | Feeds a `WIIUREC3` snapshot (`RecordingSnapshot::parse`) through the shipping planner and reports its outcomes, searches, time and the objects left at N, overall, by shader and by frame, so planner versions are compared on the same frames. Maintainer tool. |
...
| Keeping consecutive frames for offline study | `src/wiiuport/frame/RecordingSnapshot.h` | Armed over the channel; copies the next K frames' assemblies and frames them as `WIIUREC3`, and reads them back (`parse`). `tools/wiiuport/interpolation.py` parses them too, for the Python tools. |
...
| Reverse-engineering instruments on the running title | `src/wiiuport/guest/CallerCensus.h`, `src/wiiuport/control/GuestMemoryRead.h` | `CallerCensus` probes up to 8 title functions named by `WIIUPORT_CALLER_CENSUS` (`entry:firstInstruction` in hex, comma separated) and counts the call sites that reach each (`GET /callers`): the answer for a function reached only through a pointer. `GuestMemoryRead` answers `GET /memory?address=<hex>&size=<decimal>` (at most 16 MiB) with the guest's bytes, refusing a range that is not all guest memory. Both take the fork's calls injected; covered by `tests/cxx/control_tests.cpp`. |
