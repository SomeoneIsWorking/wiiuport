...
| Which of the title's own objects a draw drew | `src/wiiuport/interp/DrawObjects.h`; title side `src/wiiuport/guest/ParticleProbe.h`, `WaveProbe.h`, `LineProbe.h` (read guest memory through the fork), `BufferWriters.h` (pure) | `DrawObjects` names the title's object (guest address, and an age where the title renews it in place) behind a draw's vertex buffer; `VertexBlend` partners such a draw with the same object's draws that continue as it (`GuestObject::continuesAs`). `ParticleProbe` watches Wind Waker HD's particle commit (`0x02825158`, called by the ripple draw and every JPA draw executor) and `WaveProbe` its vertex-buffer flush (`0x027b5e94`) from the sea-wave writer's call site, and `LineProbe` the double-buffer helper (`0x027ff1d8`) from the 3D lines' updates; all record into `BufferWriters` each object's buffer and the leading bytes it wrote there, which verifies a draw by its first 80 bytes being those. Counted in `GET /interpolation` (`bufferWriters`, `guestObjectDraws`); covered by `tests/cxx/buffer_writers_tests.cpp`. |
