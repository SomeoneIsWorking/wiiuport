...
ContinuousInterpolator::ContinuousInterpolator(const ViewTracker& tracker,
    ...
    : m_tracker(tracker), m_substitution(substitution), m_objects(objects), m_replayer(replayer),
      m_presenter(presenter), m_scheduler(scheduler), m_now(now) {
}
...
void ContinuousInterpolator::restoreGuestFrame(const frame::FrameRecording& recording) {
    m_replayer.armOnce();
    if (m_replayer.replayIfArmed(recording) == 0 || !m_presenter.copyNow()) {
        ++m_restoresRefused;
    }
}
