...
VertexChanges::Census VertexChanges::census() const {
    std::lock_guard lock(m_mutex);
    return {m_censusAsked, m_censusTaken, m_withUniforms.counts()};
}

void VertexChanges::Tally::add(const VertexShader& shader, uint64_t hash, uint64_t bytes) {
...
void VertexChanges::Tally::compareFrame() {
    ...
    for (auto& [shader, hashes] : m_previous) {
...
void VertexChanges::Tally::clear() {
    m_counts.clear();
    m_frame.clear();
    m_previous.clear();
}
